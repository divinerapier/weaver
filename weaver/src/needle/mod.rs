...
impl NeedleHeader {
    ...
    pub fn as_bytes(&self) -> Bytes {
        ...
        byteorder::LittleEndian::write_u32(&mut buffer[0..4], self.magic_number);
        byteorder::LittleEndian::write_u32(&mut buffer[4..8], self.cookie);
        byteorder::LittleEndian::write_u64(&mut buffer[8..16], self.needle_id);
        byteorder::LittleEndian::write_u32(&mut buffer[16..20], self.alternate_key);
        byteorder::LittleEndian::write_u16(&mut buffer[20..22], self.flags);
        byteorder::LittleEndian::write_u32(&mut buffer[22..26], self.size);
        Bytes::from(buffer)
    }
}
...
impl NeedleFooter {
    ...
    pub fn as_bytes(&self) -> Bytes {
        ...
        byteorder::LittleEndian::write_u32(&mut buffer[0..4], 0);
        byteorder::LittleEndian::write_u32(&mut buffer[4..8], 0);
        byteorder::LittleEndian::write_u32(&mut buffer[8..12], self.padding);
        Bytes::from(buffer)
    }
}
...
impl From<Vec<u8>> for NeedleHeader {
    fn from(v: Vec<u8>) -> NeedleHeader {
        assert!(v.len() >= 26);
        NeedleHeader {
            magic_number: byteorder::LittleEndian::read_u32(&v[0..4]),
            cookie: byteorder::LittleEndian::read_u32(&v[4..8]),
            needle_id: byteorder::LittleEndian::read_u64(&v[8..16]),
            alternate_key: byteorder::LittleEndian::read_u32(&v[16..20]),
            flags: byteorder::LittleEndian::read_u16(&v[20..22]),
            size: byteorder::LittleEndian::read_u32(&v[22..26]),
        }
    }
}
